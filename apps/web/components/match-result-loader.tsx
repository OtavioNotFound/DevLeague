...
export function MatchResultLoader({ id, demoOutcome }: { id: string; demoOutcome?: string }) {
  ...
  const description = result.reason === 'VOID_SYSTEM' ? 'A partida foi anulada por uma falha sistêmica e nenhum rating foi alterado.' : result.reason === 'DRAW_TIMEOUT' ? 'O tempo terminou sem uma solução aceita.' : won ? 'Sua solução foi a primeira resposta aceita pelo servidor.' : result.reason === 'FORFEIT' ? 'A partida terminou por desistência.' : 'Seu rival teve a primeira solução aceita pelo servidor.';

  return <><section className={won ? 'result-hero won' : 'result-hero lost'}>{won ? <Trophy size={48} /> : <Code2 size={48} />}<p className="eyebrow">PARTIDA ENCERRADA</p><h1>{title}</h1><p>{description}</p><div className="result-versus"><span><b>{account.username.slice(0,2).toUpperCase()}</b><strong>{account.username}</strong><small>{won ? 'VENCEDOR' : draw ? 'EMPATE' : 'FINALIZADO'}</small></span><i>{draw ? '—' : won ? '1 — 0' : '0 — 1'}</i><span><b>{opponent?.username.slice(0,2).toUpperCase() ?? '??'}</b><strong>{opponent?.username ?? 'rival'}</strong><small>{!won && !draw ? 'VENCEDOR' : 'FINALIZADO'}</small></span></div></section><section className="result-stats"><article><Gauge size={19} /><span><small>RATING</small><strong>{mine ? `${mine.before} → ${mine.after}` : `${account.rating}`}</strong></span><b className={(mine?.delta ?? 0) >= 0 ? 'positive' : 'negative'}>{formatDelta(mine?.delta ?? 0)}</b></article><article><Code2 size={19} /><span><small>SEUS ENVIOS</small><strong>{match.mySubmissions.length} tentativas</strong></span><b>{match.type === 'RANKED_PUBLIC' ? 'Ranked' : 'Unranked'}</b></article><article><Trophy size={19} /><span><small>DECISÃO</small><strong>{reasonLabel(result.reason)}</strong></span><b>{new Date(result.finishedAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</b></article></section><div className="result-actions"><Link className="button primary large" href="/play/queue"><RotateCcw size={18} /> Jogar novamente</Link><Link className="button secondary large" href="/practice"><Code2 size={18} /> Praticar</Link><Link className="text-link" href="/home"><Home size={16} /> Voltar ao início <ArrowRight size={15} /></Link></div><p className="match-id">Partida {id} · resultado baseado na ordem de admissão registrada pelo servidor</p></>;
}
